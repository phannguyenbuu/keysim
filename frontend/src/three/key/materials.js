...
const setMaterialIndexes = (mesh, side, top, isoent) => {
  ...
  // Đảm bảo geometry có groups, nếu chưa tạo thì tạo mặc định
  if (!mesh.geometry.groups || mesh.geometry.groups.length === 0) {
    ...
    // giả sử mỗi 3 index là 1 face, tạo nhóm mặc định
  ...
  // Duyệt các nhóm (faces) và gán materialIndex
  mesh.geometry.groups.forEach((group, i) => {
    let isTop = i < threshold || i === 8;
    group.materialIndex = isTop ? top : side;
  });
